...
2024-10-02  Clean Database Block Code
            - Added DROP/CREATE-TRUNCATE/INSERT/SELECT TABLE 
            
-- -------------------------------------------------------------------

2024-11-04  Clean Database Block Code
            - Added DROP/CREATE-TRUNCATE/INSERT/SELECT geo TABLES
            - Altered and Updated query for people TABLE
            - Made Envelope Address Format Query
            
-- -------------------------------------------------------------------

2024-11-25  Receipt/Sales Logic and Methodology
            - Added manufacturer, people_employee, item
            , item_price, item_type,item_inventory
            , order, order__item and taxes TABLES
            - created JOINs
            - Created 6 receipt views
...
CREATE TABLE IF NOT EXISTS geo_country (
  ...
  , CONSTRAINT gco__UK_abbr UNIQUE (co_code ASC)
);
...
SELECT gco.co_id, gco.co_name, gco.co_code
     , gco.active
...
VALUES                 ('Manitoba',   'MB', 1)
                     ...
                     , ('Texas',      NULL, 4)
                     , ('Washington', NULL, 4);
SELECT grg.rg_id, grg.rg_name, grg.rg_code, co_id
     , grg.active
FROM   geo_region grg
WHERE  grg.active=1;
...
-- gr JOIN to gc
SELECT grg.rg_id, grg.rg_name, grg.rg_code, grg.co_id
     , gco.co_id, gco.co_name, gco.co_code
FROM   geo_region grg
...
SELECT gtc.tc_id, gtc.tc_name, gtc.rg_id
     , grg.rg_id, grg.rg_name, grg.rg_code, grg.co_id
     , gco.co_id, gco.co_name, gco.co_code
FROM   geo_towncity gtc
...
CREATE VIEW z__people__meta_load AS (
    ...
    SELECT p.p_id
        ...
        , CONCAT('1204', LPAD(FLOOR(RAND() * 10000000), 7, '0')) AS phone_pri
        , CONCAT('1204', LPAD(FLOOR(RAND() * 10000000), 7, '0')) AS phone_sec
        , CONCAT('1204', LPAD(FLOOR(RAND() * 10000000), 7, '0')) AS phone_fax
        , NULL AS addr_suite
        , MIN(
            CONCAT(
                 CASE  WHEN RAND() < 0.25 THEN FLOOR(RAND() * 100)
                       WHEN RAND() < 0.50 THEN FLOOR(RAND() * 1000)
                       WHEN RAND() < 0.75 THEN FLOOR(RAND() * 10000)
                                          ELSE FLOOR(RAND() * 10) + 10
                 END
                 ,' ', zs.street_name)
          ) AS addr
        , CONCAT(
...
UPDATE people
SET addr_suite=CASE WHEN RAND() < 0.33 THEN FLOOR(RAND() * 100)
                    WHEN RAND() < 0.67 THEN FLOOR(RAND() * 1000)
                                       ELSE FLOOR(RAND() * 10) + 10
               END
...
-- final PEOPLE TABLE query
SELECT  p.p_id
      , p.first_name, p.last_name, p.email_addr, p.password 
      
      , CONCAT(
             '+', MID(p.phone_pri, 1,1), '(', MID(p.phone_pri, 2, 3)
           , ')', MID(p.phone_pri, 5,3), '-', MID(p.phone_pri, 8, 4)
        ) AS phone_pri
      , CONCAT(
             '+', MID(p.phone_sec, 1,1), '(', MID(p.phone_sec, 2, 3)
           , ')', MID(p.phone_sec, 5,3), '-', MID(p.phone_sec, 8, 4)
        ) AS phone_sec
      , CONCAT(
             '+', MID(p.phone_fax, 1,1), '(', MID(p.phone_fax, 2, 3)
           , ')', MID(p.phone_fax, 5,3), '-', MID(p.phone_fax, 8, 4)
        ) AS phone_fax
...
SELECT  p.p_id
      ...
      , p.addr_suite, p.addr, p.addr_code , p.addr_info, p.addr_delivery
      , CASE
          WHEN gc.co_code='CA' THEN CONCAT(LEFT(p.addr_code,3),' ',RIGHT(p.addr_code,3))
          WHEN gc.co_code='JP' THEN CONCAT(
                                         CONVERT(UNHEX('E38092') USING utf8mb4)
                                       , LEFT(p.addr_code,3),'-', RIGHT(p.addr_code,4)
                                    )
          WHEN gc.co_code='KR' THEN CONCAT(LEFT(p.addr_code,3),' ',RIGHT(p.addr_code,3))
          -- WHEN gc.co_code='US' THEN p.addr_code -- same as below
          ELSE p.addr_code
        END AS addr_code
      , p.employee
...
SELECT p.first_name, p.last_name
    , ga.addr_type, p.addr_suite, p.addr
    , p.addr_info 
...
CREATE TABLE IF NOT EXISTS people_employee ( 
    ...
    , active    BIT NOT NULL DEFAULT 1 
    , CONSTRAINT people_employee_pk PRIMARY KEY(pe_id)
);
...
SELECT pe.pe_id, pe.p_id, pe.p_id_mgr, pe.pe_employee_id
...
-- -------------------------------------------------------------------
-- manufacturer m : DROP/CREAT-TRUNCATE/INSERT SELECT TABLE Block
...
CREATE TABLE IF NOT EXISTS manufacturer (
  ...
  , password      CHAR(32)
  , phone_pri     BIGINT    UNSIGNED     -- NOT NULL later switch
  ...
  , tc_id         MEDIUMINT UNSIGNED   -- FK geo_towncity
  ...
  , CONSTRAINT man___UK_ UNIQUE(man_name)
);
...
-- final manufacturer TABLE query
SELECT  m.p_id_man, m.man_id
      , m.man_name, m.email_addr, m.password 
      
      , CONCAT(
             '+', MID(m.phone_pri, 1,1), '(', MID(m.phone_pri, 2, 3)
           , ')', MID(m.phone_pri, 5,3), '-', MID(m.phone_pri, 8, 4)
        ) AS phone_pri
      , CONCAT(
             '+', MID(m.phone_sec, 1,1), '(', MID(m.phone_sec, 2, 3)
           , ')', MID(m.phone_sec, 5,3), '-', MID(m.phone_sec, 8, 4)
        ) AS phone_sec
      , CONCAT(
             '+', MID(m.phone_fax, 1,1), '(', MID(m.phone_fax, 2, 3)
           , ')', MID(m.phone_fax, 5,3), '-', MID(m.phone_fax, 8, 4)
        ) AS phone_fax
...
-- -------------------------------------------------------------------
-- Create the View: manufacturer_people_reps

DROP VIEW IF EXISTS manufacturer_people_reps;
CREATE VIEW manufacturer_people_reps AS

SELECT  man_id, m.man_name      
      , m.p_id_man
      , m.email_addr AS man_email_addr
      , m.password      
      , CONCAT(
             '+', MID(m.phone_pri, 1,1), '(', MID(m.phone_pri, 2, 3)
           , ')', MID(m.phone_pri, 5,3), '-', MID(m.phone_pri, 8, 4)
      ...
      , m.addr_suite, m.addr, m.addr_code, m.addr_info    
      , m.addr_delivery
      , m.addr_type_id  AS man_addr_type_id
      , m.tc_id         AS man_tc_id      
      ...
      , ga.addr_type_id, ga.addr_type
      , gt.tc_id, gt.tc_name, gt.rg_id
      , gr.rg_name, gr.rg_code, gr.co_id
      , gc.co_name, gc.co_code
FROM manufacturer m
     JOIN people p            ON m.p_id_man = p.p_id
     JOIN geo_address_type ga ON p.addr_type_id = ga.addr_type_id
     JOIN geo_towncity gt     ON p.tc_id=gt.tc_id
     ...
     JOIN geo_country gc      ON gr.co_id=gc.co_id  
WHERE 1=1
ORDER BY gc.co_name, gr.rg_name, gt.tc_name, m.man_name;

SELECT man_name, first_name, last_name
     , addr_suite, addr, addr_info
     , tc_name, rg_name, co_name, man_phone_pri
     , man_phone_sec, man_phone_fax
...
-- JOIN item to manucfacturer to item type
SELECT i.item_id, i.item_name, i.item_modelno, i.item_barcode, i.item_price
     , i.man_id, i.it_id
     , m.man_id, m.man_name
     , it.it_id, it.it_desc
...
-- JOIN orders to people to people_employee
SELECT o.order_id, o.order_no, o.order_date, o.p_id_cus, o.p_id_emp
     , p.p_id
      , p.first_name, p.last_name, p.email_addr, p.password 
      ...
      , p.addr_type_id 
      , p.tc_id
      , pe.pe_id, pe.p_id, pe.p_id_mgr, pe.pe_employee_id
      , pe.pe_hired, pe.pe_salary
FROM orders o
...
-- JOIN orders to order__item to item to item_price to item_inventory
...
SELECT oi.oi_id, oi.order_id, oi.item_id, oi.oi_qty
     ...
     , m.man_id, m.man_name
     , it.it_id, it.it_desc
     , ip.ip_id, ip.item_id, ip.item_price
     , (ip.item_price * oi.oi_qty) AS amount
     , ip.ip_beg, ip.ip_end
...
-- WHERE o.order_date BETWEEN t.tax_beg AND IFNULL(t.tax_end, o.order_date)
 GROUP BY
       oi.oi_id, oi.order_id, oi.item_id, oi.oi_qty
     , o.order_id, o.order_no, o.order_date, o.p_id_cus, o.p_id_emp
     , i.item_id, i.item_name, i.item_modelno, i.item_barcode, i.item_price
     , i.man_id, i.it_id
     , m.man_id, m.man_name
...
-- -------------------------------------------------------------------
-- First VIEW: receipt_00_header_company_info 
...
CREATE VIEW receipt_00_header_company_info AS (
    ...
    SELECT 
      ...
      , m.addr_suite, m.addr, m.addr_code
      , gt.tc_name, gr.rg_name
      , gc.co_name

    FROM 
        manufacturer m
    JOIN 
        geo_towncity gt ON m.tc_id = gt.tc_id
    JOIN 
        geo_region gr ON gt.rg_id = gr.rg_id
    JOIN 
        geo_country gc ON gr.co_id = gc.co_id
    WHERE 
        m.man_name = 'Boxstore Inc.'
    GROUP BY 
         m.man_name, m.phone_pri, m.phone_sec
       , m.addr_suite, m.addr, m.addr_code
       , gt.tc_name, gr.rg_name, gc.co_name
...
CREATE VIEW receipt_01_header_order_employee_customer AS (
    ...
    SELECT o.order_id AS order_ID
         ...
         , emp_emp.pe_employee_id AS employee_ID
         , CONCAT(
                cus.first_name, ' ', cus.last_name
                 ) AS customer_name
    FROM orders o
         JOIN people AS emp ON o.p_id_emp=emp.p_id
         JOIN people_employee AS emp_emp ON emp.p_id=emp_emp.p_id
         JOIN people AS cus ON o.p_id_cus=cus.p_id
);
...
SELECT o.order_id AS order_ID, o.order_no AS order_NO, o.order_date AS order_date
     , m.man_name AS manufacturer_name
     , i.item_id AS item_ID
     , i.item_name, it.it_desc AS item_description
     , ip.item_price
     , oi.oi_qty AS order_item_quantity
     , (ip.item_price * oi.oi_qty) AS total_price
     , GROUP_CONCAT(ii.ii_serialno ORDER BY ii.ii_id ASC) AS serial_numbers
FROM orders o
...
SELECT order_id, order_no, order_date, subtotal
...
SELECT rs.order_id , rs.order_date 
     , rs.subtotal , t.tax_perc AS tax_percentage
     , (rs.subtotal * t.tax_perc / 100) AS tax_amount
FROM receipt_20_footer_subtotal rs
CROSS JOIN taxes t
WHERE rs.order_date BETWEEN t.tax_beg AND IFNULL(t.tax_end, rs.order_date)
;
...
SELECT order_id AS order_ID
     , subtotal, SUM(tax_amount) AS tax
     , subtotal + SUM(tax_amount) AS grand_total
FROM receipt_21_footer_subtotal_taxes
GROUP BY order_id
       , subtotal
;

SELECT order_ID, tax, grand_total
